...
export function LogoMark({ className = "size-10" }: { className?: string }) {
  // Id unici per ogni logo. Il marchio compare due volte nella pagina
  // (intestazione e piede): con id fissi i due SVG si contendono gli stessi
  // gradienti, e dove il primo e' nascosto il secondo resta senza colore.
  const id = useId();
  const goccia = `${id}-goccia`;
  const nastro = `${id}-nastro`;
  const dentro = `${id}-dentro`;
  return (
    <svg viewBox="0 0 64 64" className={className} aria-hidden="true">
      <defs>
        <linearGradient id={goccia} x1="14" y1="8" x2="50" y2="58" gradientUnits="userSpaceOnUse">
          <stop offset="0" stopColor="#56B4EE" />
          <stop offset="0.5" stopColor="#1B84DD" />
          <stop offset="1" stopColor="#1663B0" />
        </linearGradient>
        <linearGradient id={nastro} x1="20" y1="14" x2="44" y2="52" gradientUnits="userSpaceOnUse">
      ...
      <g clipPath={`url(#${dentro})`}>
        <path d="M6 44 L40 2 L52 10 L18 54 Z" fill={`url(#${nastro})`} opacity="0.55" />
      </g>
    </svg>
  );
}
